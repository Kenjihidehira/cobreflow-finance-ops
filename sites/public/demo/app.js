const filters = { status: "all", channel: "all", search: "" };
...
function render(payload) {
  ...
  qs("#receivablesBody").innerHTML = receivables.receivables.map((item) => {
  ...
  qs("#automationQueue").innerHTML = automations.queue.slice(0, 5).map((item) => `<article class="queue-item"><header><strong>${item.customer}</strong><span class="risk-pill ${riskClass(item)}">${item.riskScore}</span></header><p>${item.action} via ${channelLabels[item.channel] || item.channel}</p><p>${item.invoiceId} - ${money.format(item.balance)} - ${item.reason}</p></article>`).join("");
...
qs("#searchInput").addEventListener("input", (event) => { filters.search = event.target.value; loadDashboard().catch(showError); });
